...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      console.warn("Key error in NutriScan AI API Route: GEMINI_API_KEY is missing.");
      return res.status(503).json({
        error: "API Key Not Found",
        message: "Please add your GEMINI_API_KEY environment variable on Vercel.",
      });
    }
